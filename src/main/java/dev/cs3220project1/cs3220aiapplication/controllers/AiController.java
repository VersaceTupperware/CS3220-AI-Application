...
//    private final DataStore dataStore;
    ...
    public String generate(
        ...
        try {
            Meal finalMeal = generateMealFromAi(username, mealType, servings, calories, prefs, note);
        ...
        return "ai/assistant";  // full page render
    }

    @PostMapping("/ai/card")
    public String generateCardFragment(
            @RequestParam(required = false) String mealType,
            @RequestParam(required = false) Integer servings,
            @RequestParam(required = false) Integer calories,
            @RequestParam(required = false, name = "pref") List<String> prefs,
        ...
        try {
            // render meal card only
            Meal finalMeal = generateMealFromAi(username, mealType, servings, calories, prefs, note);
            model.addAttribute("meal", finalMeal);
            return "ai/mealcard";
        } catch (Exception e) {
            model.addAttribute("error", "Failed to generate: " + e.getMessage());
            return "ai/error-fragment";
        }
    }

    private Meal generateMealFromAi(
            String username,
            String mealType,
            Integer servings,
            Integer calories,
